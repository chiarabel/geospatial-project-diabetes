...
```
Given the absence of significant $\rho$ and $\lambda$ parameters, the statistical relevance of spatially lagged explanatory variables, and the lowest AIC value among competing models, the SLX specification is selected as the preferred model.
...
```{r}
# Moran's I on SLX
moran.test(residuals(SLX), lw_q, zero.policy = TRUE)
